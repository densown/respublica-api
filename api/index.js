...
app.get("/api/gesetze", asyncHandler(async (req, res) => {
...
app.get("/api/gesetze/stats", asyncHandler(async (_req, res) => {
...
app.get("/api/gesetze/:id", asyncHandler(async (req, res) => {
...
app.get("/api/abstimmungen/latest", asyncHandler(async (req, res) => {
...
app.get("/api/bundestag/abgeordnete", asyncHandler(async (_req, res) => {
  const [rows] = await getPool().query(
...
app.get("/api/bundestag/abgeordnete/:id", asyncHandler(async (req, res) => {
...
app.get("/api/abgeordnete", asyncHandler(async (_req, res) => {
  const [rows] = await getPool().query(
...
app.get("/api/abgeordnete/:aw_id/votes", asyncHandler(async (req, res) => {
...
app.get("/api/bundestag/abstimmungen", asyncHandler(async (_req, res) => {
  const [rows] = await getPool().query(
    `SELECT DISTINCT poll_id, poll_titel, poll_datum
     FROM abstimmungen
     ORDER BY poll_datum DESC`
...
app.get("/api/bundestag/abstimmungen/:pollId", asyncHandler(async (req, res) => {
...
app.get("/api/bundestag/poll-votes/:poll_id", asyncHandler(async (req, res) => {
...
app.get("/api/abstimmungen/:poll_id", asyncHandler(async (req, res) => {
  const pollId = Number.parseInt(req.params.poll_id, 10);
  if (!Number.isFinite(pollId)) {
    res.status(400).json({ error: "Ungültige poll_id" });
    return;
...
/** Urteile: Liste */
app.get("/api/urteile", asyncHandler(async (req, res) => {
...
/** Urteile: Einzeln */
app.get("/api/urteile/:id", asyncHandler(async (req, res) => {
  const id = Number.parseInt(req.params.id, 10);
...
app.get("/api/eu-recht/stats", asyncHandler(async (_req, res) => {
  const pool = getPool();
...
app.get("/api/eu-recht", asyncHandler(async (req, res) => {
...
app.get("/api/eu-recht/:id", asyncHandler(async (req, res) => {
  const id = Number.parseInt(req.params.id, 10);
  if (!Number.isFinite(id)) {
    res.status(400).json({ error: "Ungültige id" });
    return;
...
app.get("/api/eu-urteile/stats", asyncHandler(async (_req, res) => {
  const pool = getPool();
  const [[{ total }]] = await pool.query(
...
app.get("/api/eu-urteile", asyncHandler(async (req, res) => {
  const gericht = req.query.gericht || null;
  const rechtsgebiet = req.query.rechtsgebiet || null;
  const search = req.query.search || null;
  let limit = Number.parseInt(String(req.query.limit || "50"), 10);
...
app.get("/api/eu-urteile/:id", asyncHandler(async (req, res) => {
  const id = Number.parseInt(req.params.id, 10);
  if (!Number.isFinite(id)) {
    res.status(400).json({ error: "Ungültige id" });
    return;
...
app.get("/api/lobbyregister", asyncHandler(async (req, res) => {
...
app.get("/api/lobbyregister/stats", asyncHandler(async (_req, res) => {
  const pool = getPool();
...
app.get("/api/lobbyregister/by-field", asyncHandler(async (_req, res) => {
  const [rows] = await getPool().query(
    `SELECT
       foi.code AS code,
       foi.de AS de,
...
app.get("/api/lobbyregister/by-city", asyncHandler(async (_req, res) => {
  const [rows] = await getPool().query(
    `SELECT
       city,
       country,
...
app.get("/api/lobbyregister/by-time", asyncHandler(async (_req, res) => {
  const [rows] = await getPool().query(
    `SELECT
...
app.get("/api/lobbyregister/:register_number/projects", asyncHandler(async (req, res) => {
...
app.get("/api/lobby-projects/by-gesetz-id", asyncHandler(async (req, res) => {
...
app.get("/api/lobbyregister/:register_number/gesetze", asyncHandler(async (req, res) => {
  const registerNumber = String(req.params.register_number || "").trim();
  if (!registerNumber) {
...
app.get("/api/lobby-projects/by-law", asyncHandler(async (req, res) => {
  const q = String(req.query.q ?? "").trim();
  if (!q || q.length < 2) {
...
app.get("/api/lobby-projects/stats", asyncHandler(async (_req, res) => {
  const [rows] = await getPool().query(
...
app.get("/api/lobbyregister/:register_number", asyncHandler(async (req, res) => {
  const registerNumber = String(req.params.register_number || "").trim();
  if (!registerNumber) {
    res.status(400).json({ error: "Ungültige register_number" });
    return;
...
app.get("/api/wahlen/years", asyncHandler(async (req, res) => {
...
app.get("/api/wahlen/map", asyncHandler(async (req, res) => {
  const typ = wahlenParseTyp(req.query.typ);
...
app.get("/api/wahlen/timeseries", asyncHandler(async (req, res) => {
  const typ = wahlenParseTyp(req.query.typ);
...
app.get("/api/wahlen/compare", asyncHandler(async (req, res) => {
  const typ = wahlenParseTyp(req.query.typ);
  const party = String(req.query.party || "").trim().toLowerCase();
...
app.get("/api/wahlen/scatter", asyncHandler(async (req, res) => {
  const typ = wahlenParseTyp(req.query.typ);
  const year = Number.parseInt(String(req.query.year || ""), 10);
...
app.get("/api/wahlen/ranking", asyncHandler(async (req, res) => {
  const typ = wahlenParseTyp(req.query.typ);
  const year = Number.parseInt(String(req.query.year || ""), 10);
  const party = String(req.query.party || "").trim().toLowerCase();
...
app.get("/api/wahlen/change", asyncHandler(async (req, res) => {
  const typ = wahlenParseTyp(req.query.typ);
  const party = String(req.query.party || "").trim().toLowerCase();
...
app.get("/api/wahlen/national-average", asyncHandler(async (req, res) => {
  const typ = wahlenParseTyp(req.query.typ);
  const party = String(req.query.party || "").trim().toLowerCase();
...
app.get("/api/wahlen/stats", asyncHandler(async (_req, res) => {
  const pool = getPool();
...
app.get("/api/wahlen/region/:ags", asyncHandler(async (req, res) => {
...
app.get("/api/world/categories", asyncHandler(async (req, res) => {
...
app.get("/api/world/indicators", asyncHandler(async (req, res) => {
  const lang = worldLang(req);
  const nameCol = worldNameCol(lang);
  const unitCol = worldUnitCol(lang);
  const [rows] = await getPool().query(
...
app.get("/api/world/map", asyncHandler(async (req, res) => {
...
app.get("/api/world/country/:code", asyncHandler(async (req, res) => {
...
app.get("/api/world/climate/:iso3", asyncHandler(async (req, res) => {
...
app.get("/api/world/timeseries", asyncHandler(async (req, res) => {
...
app.get("/api/world/compare", asyncHandler(async (req, res) => {
...
app.get("/api/world/ranking", asyncHandler(async (req, res) => {
  const indicator = String(req.query.indicator || "").trim();
  let year = Number.parseInt(String(req.query.year || ""), 10);
...
app.get("/api/world/scatter", asyncHandler(async (req, res) => {
...
app.get("/api/world/sources", asyncHandler(async (_req, res) => {
  const pool = getPool();
  const [rows] = await pool.query(
    `SELECT s.id,
            s.slug,
...
app.get("/api/world/stats", asyncHandler(async (_req, res) => {
  const pool = getPool();
...
app.get("/api/world/trade/:iso3", asyncHandler(async (req, res) => {
  const iso3 = String(req.params.iso3 || "")
    .trim()
    .toUpperCase()
    .slice(0, 3);
...
app.get("/api/world/trade/:iso3/timeseries", asyncHandler(async (req, res) => {
  const iso3 = String(req.params.iso3 || "")
    .trim()
    .toUpperCase()
    .slice(0, 3);
...
// 404-Fallback + zentrale Fehlerbehandlung (nach allen Routen registrieren)
